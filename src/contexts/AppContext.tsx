...
import { appContextReducer, APP_CONTEXT_ACTIONS } from "./AppContextReducer";
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const setShowPopOver = (bool: boolean) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_SHOW_POPOVER, bool));
  };

  const updateMyAccountQty = (qty: number) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_MY_ACCOUNT_QTY, qty));
  };

  const setGlobalError = (error: string | null) => {
    if (globalError !== error) {
      dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_GLOBAL_ERROR, error));
    }
  };

  const setShowPayNow = (bool: boolean) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_SHOW_PAY_NOW, bool));
  };
  ...
  const updateAnonymousId = (id: string) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_ANONYMOUS_ID, id));
  };
  ...
  const updateQuantity = (quantity: number) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_QUANTITY, quantity));
  };

  const updateUserAuthToken = (id: string | null) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_USER_TOKEN, id));
  };

  const togglePromoCode = (promoObj: TOrderReceiptCouponRedemption) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.TOGGLE_PROMO, promoObj));
  };

  const addItemToCart = (newProduct: object) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_PRODUCT, newProduct));
  };

  const addOrderItems = (newProps: object) => {
    const newItems = { ...(currentCart?.orderItems as CartItem), ...newProps };
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_ITEMS, newItems));
  };

  const updateCurrentExtra = (extra: number) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_CURRENT_EXTRA, extra));
  };

  const updateCurrentPage = (page: string) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_CURRENT_PAGE, page));
  };

  const updateCustomerDetails = (name: string, value?: string | object) => {
    dispatch(
      actionCreator(APP_CONTEXT_ACTIONS.UPDATE_CART, {
        ...currentCart,
        [name]: value,
      })
    );
  ...
  const updateOrder = (orderToUpdate: UpdateOrderProps) => {
    ...
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_ORDER, orderToUpdate));
  };

  const setShowScatulator = (bool: boolean) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_SHOW_SCATULATOR, bool));
  };

  const updateShowVerification = (bool: boolean) => {
    dispatch(
      actionCreator(APP_CONTEXT_ACTIONS.UPDATE_SHOW_VERIFICATION, bool)
    );
  };
  ...
  const setBraintreeInstance = (instance: Dropin | undefined) => {
    dispatch(
      actionCreator(APP_CONTEXT_ACTIONS.UPDATE_BRAINTREE_INSTANCE, instance)
    );
  };

  const setPaymentToken = (token: string) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_PAYMENT_TOKEN, token));
  };

  const setIsScrollActivated = (bool: boolean) => {
    dispatch(
      actionCreator(APP_CONTEXT_ACTIONS.UPDATE_IS_SCROLL_ACTIVATED, bool)
    );
  };

  const setGlobalMessage = (messageObj: { show: boolean; message: string }) => {
    dispatch(
      actionCreator(APP_CONTEXT_ACTIONS.SET_GLOBAL_MESSAGE, messageObj)
    );
  };
  ...
  const updateCustomerDetailsAtOnce = (
    ...
    dispatch(
      actionCreator(
        APP_CONTEXT_ACTIONS.UPDATE_CART,
        localCurrentCart?.cartItems
          ? {
            ...localCurrentCart,
            ...customerDetails,
          }
  ...
  const updateLocalAddress = (newAddress: LocalAddressProps) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_ADDRESS, newAddress));
  };

  const updateOrderPlaced = (bool: boolean) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_ORDER_PLACED, bool));
  };

  const updateShowPhoneError = (message: string) => {
    dispatch(
      actionCreator(APP_CONTEXT_ACTIONS.UPDATE_SHOW_PHONE_ERROR, message)
    );
  };

  const updateMethod = (method: "phone" | "email" | null) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_METHOD, method));
  };

  const updateCustomer = (customer: UpdateCustomerProps | null) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_CUSTOMER, customer));
  };

  const updateShowPhoneLogin = (bool: boolean) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_SHOW_PHONE_LOGIN, bool));
  };

  const setEmailChecked = (bool: boolean) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_EMAIL_CHECKED, bool));
  };

  const setPhoneChecked = (bool: boolean) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_PHONE_CHECKED, bool));
  };

  const updateSendingCodeToNewUser = (bool: boolean) => {
    dispatch(
      actionCreator(APP_CONTEXT_ACTIONS.UPDATE_SENDING_CODE_TO_NEW_USER, bool)
    );
  };

  const setShowCart = (bool: boolean) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_SHOW_CART, bool));
  };

  const updateUser = (user: object | null) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.UPDATE_USER, user));
  };
  ...
  const setCurrentTab = (newTab: string) => {
    dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_CURRENT_TAB, newTab));
  };
  ...
  const fetchCart = async () => {
    ...
    if (response.success) {
      dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_CART, response.data));
    }
  };
  ...
  const fetchProduct = async () => {
    ...
    if (success) {
      dispatch(actionCreator(APP_CONTEXT_ACTIONS.SET_PRODUCT, product));
    }
  };
